...
export const harness = new Harness(passport, [
  Harness.determinism(run, [team, { members: [] }, { members: [member(1, 1, 1), member(0, 0, 0)] }]),
  Harness.edges(run, [
    ...
    { name: 'identical', input: { members: [member(0.8, 0.5, 0.7), member(0.8, 0.5, 0.7)] }, expect: (o, a) => a(Math.abs(o.Gcc.value - 0.4) < 1e-6, 'одинаковые: Gcc = Cr·Sr = 0,4') },
    { name: 'one-invalid', input: { members: [member(0.8, 0.5, 0.7), member(1.2, 0.5, 0.7)] }, expect: (o, a) => a(o.status === 'missing', 'Cr > 1 → ⛔') },
    { name: 'extremes', input: { members: [member(0, 0, 0), member(1, 1, 1)] }, expect: (o, a) => a(o.Gcc.value === 0, 'нулевой участник → Gcc = 0') }
  ]),
  ...
  Harness.stress(run, (rng) => ({ members: Array.from({ length: 2 + Math.floor(rng.between(0, 9)) }, () => member(rng.next(), rng.next(), rng.next())) }), 1000, 2, 5),
